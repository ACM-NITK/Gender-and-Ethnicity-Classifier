...
file=[]
file1="25_1_3_20170117151955147.jpg"
file2="9_0_1_20170113175830459.jpg"
file.append(file1)
file.append(file2)
...
labels1 = ['White','Black','Asian','Indian','Others']
labels2 = ["Male","Female"]
eth_model = pickle.load(open("data1.pkl", 'rb'))
age_model = pickle.load(open("data.pkl", 'rb'))
g_model = pickle.load(open("data43.pkl",'rb'))
y_eth=eth_model.predict(X)
y_age=age_model.predict(X)
y_gen=g_model.predict(X)


for i in range(2):
	predict_index_eth = np.argmax(y_eth[i])
	print((labels1[predict_index_eth]))

	predict_index_age = np.argmax(y_age[i])
	print(predict_index_age*10,"-",predict_index_age*10+10)

	predict_index_g = np.argmax(y_gen[i])
	print(predict_index_g)
